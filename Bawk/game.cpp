...
int Game::init() {
    ...
    player->load_selfs();
    last_player_pos = player->get_center_pos();
...
void Game::render_lights() {
    ...
    fmat4 transform(1);
    world->render_lights(&transform, player->get_center_pos());
    if (bar->get_current()) {
        bar->get_current()->render_light_in_world(&transform, player->get_center_pos());
    }
}
...
void Game::check_need_update() {
    fvec3 player_pos = player->get_center_pos();
